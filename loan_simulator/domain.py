...
def calc(parcel: str, interest_rate: str, due_day: str, discharge_date: str, parcels_amount: str) -> float:
    ...
    result = regex.findall(str(response.content))[0].strip().replace('.', '').replace(',', '.')
    return float(result)
